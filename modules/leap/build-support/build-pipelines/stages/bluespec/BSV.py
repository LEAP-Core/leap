...
class BSV():
  ...
  def __init__(self, moduleList):
    ...
    if not moduleList.isDependsBuild:
      ...
      def cut_tree_build(target, source, env):
              ...
              tree_file = open(get_build_path(moduleList, moduleList.topModule) + "/build_tree.bsv",'w')
      ...
      if use_tree_build:
          build_path = get_build_path(moduleList, moduleList.topModule)
          temp_path = get_temp_path(moduleList, moduleList.topModule)
  ...
  def build_synth_boundary(self, moduleList, module):
    ...
    def compile_bo(source, target, env, for_signature):
      cmd = ''
      if (str(source[0]) != get_build_path(moduleList, moduleList.topModule) + '/' + moduleList.topModule.name + '.bsv'): 
