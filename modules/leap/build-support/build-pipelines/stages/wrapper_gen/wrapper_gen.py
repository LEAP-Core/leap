...
def generateSynthWrapper(liModule,
     ...
     _emitSynthWrapper(liModule,
                       ...
                       synthBoundaryModule = synthBoundaryModule)
...
def _emitSynthWrapper(liModule,
    ...
    synthHandle.write("//Generated by wrapper_gen.py\n")
    ...
    # If we renamed the module, the .bo file will be different. We reflect that 
    # here.
    if(synthBoundaryModule is None): 
        synthHandle.write('import ' + liModule.name + '_Wrapper::*;//Fixme\n')      
    else:
        synthHandle.write('import ' + synthBoundaryModule.name + '_Wrapper::*;//Fixme\n')      
...
def separateScratchpadChains(chains):
    ...
    clients = []
    for chain in chains: 
        m = re.match("(.*)_(?:Req|Resp)_(\w+)", chain)
        if m:
            client = (m.group(1), m.group(2))
...
#
def connectScratchpadChains(controllers, clients):
    network = {}
    ringstops = []
...
#
def genRemapWrapper(network, fileHandle):
    ...
    for controller in network: 
        ...
        for client in clients:
            fileHandle.write("    if (inputName == \"" + client[0] + "_Req_" + client[1] + "\")\n    begin\n")
            fileHandle.write("        outputName = \"" + controller + "_Req\";\n    end\n")
            fileHandle.write("    else if (inputName == \"" + client[0] + "_Resp_" + client[1] + "\")\n    begin\n")
...
#
def remapScratchpadConnections(liModules, fileHandle):
    ...
    network = connectScratchpadChains(controllers, clients)
    print network
    genRemapWrapper(network, fileHandle)
...
class WrapperGen():
  ...
  def __init__(self, moduleList):
    ...
    if(first_pass_LI_graph is None):
        ...
        remapFilePath = moduleList.env['DEFS']['ROOT_DIR_HW'] + '/' + moduleList.topModule.buildPath + '/soft_connection_remap.bsh' 
        remapHandle = open(remapFilePath, 'w')
        remapScratchpadConnections(first_pass_LI_graph.modules.values(), remapHandle) 
        remapHandle.close();
