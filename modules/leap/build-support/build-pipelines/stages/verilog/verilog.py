...
#        ' ' + moduleList.env['DEFS']['BDPI_CS']
    ...
    # iverilog command line by analyzing its compiled script. 
    def generate_vexe_wrapper(target, source, env):
        ...
        wrapper_handle.write('@script = `cat ' + TMP_BSC_DIR + '/' + APM_NAME + '_hw.exe' + '`;\n')   
        wrapper_handle.write('$script[0] =~ s/#!/ /g;\n')
        wrapper_handle.write('$vvp = $script[0];\n')
        wrapper_handle.write('chomp($vvp);\n')
        wrapper_handle.write('`$vvp -m$ENV{"PLATFORM_DIRECTORY"}/directc_mk_model_Wrapper.so ' + TMP_BSC_DIR + '/' + APM_NAME + '_hw.exe' + ' +bscvcd \$* `;\n')
